#!/bin/bash
# ****************************************************************************
# ** [JP]  ファイル名 : aijbLogServerStart.sh                                    **
# ** [JP]  処理概要   : AijbLogServerの起動                                      **
# ** [JP]                                                                       **
# ** [JP]    AijbLogServerの起動はバックグラウンドで実行します。                 **
# ** [JP]                                                                       **
# ** [JP]  引数       : なし                                                     **
# ** [JP]  戻り値     : 0(正常終了)/16(異常終了)                                 **
# **                                                                        **
# ** [EN]  File name             : aijbLogServerStart.sh                         **
# ** [EN]  Outline of processing : Starting AijbLogServer                        **
# ** [EN]                                                                       **
# ** [EN]    AijbLogServer start to run in the background.                       **
# ** [EN]                                                                       **
# ** [EN]  Argument     : None                                                   **
...
# **                                                                        **
# **  $ aijbLogServerStart.sh 4409 2013-05-23 01:46:18Z hiroko_nagata@dnitsol.com $
# **                                                                        **
# ****************************************************************************
...
# [EN] ==== Failure of common environment variables  Log output  The return value is set to 16 ========================
    echo [boot] [`date "+%Y/%m/%d %T"`] [ERROR] setenv.sh Execute Failed >> ${PRGDIR}/aijbLogServerStart.log
    exit 16
fi

# [JP] ==== ログ出力先設定 ========================================================
# [EN] ==== Log output destination setting ========================================================
LOGFILE=${AIJB_DIR}/logs/aijbLogServerStart.log

# [JP] ==== 処理開始ログ出力 ======================================================
# [EN] ==== Log output process starts ======================================================
echo `date "+%Y/%m/%d %T"` INFO :START AijbLogServer StartBatch >> $LOGFILE 2>&1
...
if [ $PROCESS_CNT -ne 0 ]; then
    echo `date "+%Y/%m/%d %T"` WARN :AijbLogServer service was already started. >> $LOGFILE 2>&1
    echo `date "+%Y/%m/%d %T"` INFO :END AijbLogServer StartBatch >> $LOGFILE 2>&1
    echo -------------------------------------------------------------------------------- >> $LOGFILE 2>&1
    exit 0
fi
