 ...
 * PROJECT：eca0027
 * 
 * TwCarryOutListPltz の検索条件Domainクラス
 * テーブル概要：Carry Out List Work For Palletize/搬出一覧(CML)ワーク  Work table to create an information manual transmission load of the case mark units shopping basket system/買い物かご方式でケースマーク単位の送荷書情報を作成するワークテーブル
 * 
...
/**
 * TwCarryOutListPltz の検索条件Domainクラスです。<BR>
 * テーブル概要：Carry Out List Work For Palletize/搬出一覧(CML)ワーク  Work table to create an information manual transmission load of the case mark units shopping basket system/買い物かご方式でケースマーク単位の送荷書情報を作成するワークテーブル<BR>
 * 
 * <BR>
 ...
 * @version $Revision$
 */
public class TwCarryOutListPltzCriteriaDomain extends AbstractDomain implements Serializable {
     ...
     * デフォルトコンストラクタ。
     */
    public TwCarryOutListPltzCriteriaDomain() {
    }
