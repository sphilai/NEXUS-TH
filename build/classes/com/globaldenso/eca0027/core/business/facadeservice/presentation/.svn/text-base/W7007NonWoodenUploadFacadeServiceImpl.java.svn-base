...
import static com.globaldenso.eca0027.core.common.constants.Eca0027Constants.NXS_E1_0044;
...
/**
 * 
 * Facade service for Non Wooden Upload action of B/L upload.
 * <br />B/LアップロードのNon Wooden Uploadアクションに対するFacadeサービスです。
 *
 * @author $Author: 400616000304 $
 * @version $Revision: 11631 $
 */
public class W7007NonWoodenUploadFacadeServiceImpl extends AbstractCreateFacadeService<W7007UpdateDomain, W7007UpdateDomain> {
     ...
     * デフォルトコンストラクタです。
     */
    public W7007NonWoodenUploadFacadeServiceImpl() {
        super();
    }
    ...
    protected void validateConsistency(W7007UpdateDomain updateDomain)
        ...
        if (!w7001ShippingActService.validateConsistencyFilePathInput(criteriaDomain.getUploadNonWdFileName())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("nonWdFileData", NXS_E7_0011, null)));
        }
        
        if (!w7001ShippingActService.validateConsistencyFileExistence(criteriaDomain.getUploadNonWdFileSize())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("nonWdFileData", GSCM_E0_0036, null)));
        }

        if (!w7001ShippingActService.validateConsistencyFileSize(criteriaDomain.getUploadNonWdFileSize())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("nonWdFileData", NXS_E7_0057, null)));
        }
        
        if (!w7001ShippingActService.validateConsistencyFileName(criteriaDomain.getUploadNonWdFileName())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("nonWdFileData", NXS_E7_0067, null)));
        }
    }
     ...
     * Call {@link W7001ShippingActService#validateDatabaseTtShippingDocumentOnOtherDocUploadSummaryNonWoodenUpload}.
     * </pre>
     * <pre>
     * ‐インボイス ロック＆取得件数チェック
     ...
     * 
     * ‐船積書類 存在チェック
     *     {@link W7001ShippingActService#validateDatabaseTtShippingDocumentOnOtherDocUploadSummaryNonWoodenUpload}を呼出す
     * </pre>
    ...
    protected void validateDatabase(W7007UpdateDomain updateDomain)
        ...
        for (int i = 0; i < listDomainList.size(); i++) {
            ...
            // 船積書類 存在チェック
            if (!w7001ShippingActService.validateDatabaseTtShippingDocumentOnOtherDocUploadSummaryNonWoodenUpload(shipperCd, invoiceNo, invoiceIssueDt)) {
                Object[] params = {
                    commonService.getResource(criteriaDomain.getLocale(), "label.nonWooden")
                };
                MessageDomain msgInvoiceNo = new MessageDomain(String.format("listDomainList[%d].invoiceNo", selectedDomain.getIdxNo()), NXS_E1_0044, params);
     ...
     * Call  {@link W7001ShippingActService#createOnOtherDocUploadSummaryNonWoodenUpload}.
     * </pre>
     * <pre>
     * ‐船積実績サービスの条件ドメイン作成
     ...
     * 
     * ‐ファイルをアップロード、再検索
     *     {@link W7001ShippingActService#createOnOtherDocUploadSummaryNonWoodenUpload}を呼出す
     * </pre>
    ...
    protected W7007UpdateDomain callServices(W7007UpdateDomain updateDomain) 
        ...
        W7001ShippingActDomain shippingActDomain = w7001ShippingActService.createOnOtherDocUploadSummaryNonWoodenUpload(shippingActCriteria);
    ...
    protected W7001ShippingActCriteriaDomain createShippingActCriteriaDomain(W7007UpdateDomain updateDomain) {
        ...
        // Upload file
        // アップロードするファイル
        shippingActCriteria.setFileData(criteriaDomain.getUploadNonWdFileData());
        shippingActCriteria.setFileName(criteriaDomain.getUploadNonWdFileName());
