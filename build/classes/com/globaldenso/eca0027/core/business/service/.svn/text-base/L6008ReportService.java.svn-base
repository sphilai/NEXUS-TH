 ...
 * <br/>帳票【 Packing Attached(MAIN)  】出力機能を提供するサービスのインターフェースです。
 * <pre>
 ...
 */
public interface L6008ReportService {

    /**
     * Get the form output information to generate a form [Packing Attached (MAIN)].
     * <br/>帳票出力情報を取得し、帳票【 Packing Attached(MAIN) 】を生成します。
     * <pre>
     ...
     *       
     * - 印刷対象のインボイスアタッチデータの取得
     *       criteriaDomainを引数に{@link RpDocDao#searchPackAtchedForMain(L6008TtPackingAttachedByHeaderAndDetailCriteriaDomain)}を呼び出します。
     *       取得件数が0であれば、エラーとします。
     *       
     * - 帳票の生成
