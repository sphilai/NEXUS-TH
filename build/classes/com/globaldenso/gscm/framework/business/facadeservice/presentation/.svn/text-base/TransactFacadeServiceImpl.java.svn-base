 ...
 *
 * $Id: TransactFacadeServiceImpl.java 5759 2014-06-27 08:14:53Z 10088DS01586 $
 *
...
/**
 * TransactFacadeServiceのテスト実装
 * Testing class for TransactFacadeService
 * 
 * <p>
 * TransactFacadeServiceのテスト実装
 * Testing class for TransactFacadeService
 ...
 */
public class TransactFacadeServiceImpl extends AbstractTransactFacadeService<Integer, TestArgDomain> {
     ...
     *
     */
    public TransactFacadeServiceImpl(){}
    ...
    protected Integer callServices(TestArgDomain filterDomain) throws ApplicationException {
        ...
        for (TestFacadeTableDomain tableDomain : tableDomainList) {
            ...
            TransactEnum tran = tranDomain.getTransact();
            if (tran == TransactEnum.INSERT) {
                testFacadeTableDao.create(tranDomain);
                count++;
            } else if (tran == TransactEnum.UPDATE) {
                count = count + testFacadeTableDao.update(tranDomain);
            } else if (tran == TransactEnum.DELETE) {
                count = count + testFacadeTableDao.delete(tranDomain);
            }
        }
        return count;
    }
