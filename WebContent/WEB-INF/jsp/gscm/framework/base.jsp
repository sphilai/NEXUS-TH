...
<!-- Concentration area of Framework base function  In case of update,update user must validate run-->
        ...
        <!--//
            ...
            GSCM_SessionTime = '<c:catch><bean:message key="sessionTime" /></c:catch>';
        ...
        <c:if test="${!empty myJs}">
            <script type="text/javascript" src="${pageContext.request.contextPath}/${myJs}"></script>
        </c:if>
        <c:if test="${!empty myCssPrefix}">
            <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/${myCssPrefix}_${sessionScope[locale_key]}.css" />
        </c:if>
...
<!-- Concentration area of Framework base End -->
        ...
        <div id="gscm_title">
            <h3>
                <bean:message key="label.testDeploy" />${screenName}
            </h3>
        </div>
        <!-- メッセージブロック -->
        <!-- Block of message -->
