...
/**
 * Interface of the service that provides the output function form [(FRT) Packing Header].
 * <br/>帳票【 Packing Header(FRT)  】出力機能を提供するサービスのインターフェースです。
 * <pre>
 * bean id:l6007ReportService
 * </pre>
 * 
 * @author $Author: 810833613 $
 * @version $Revision: 7682 $
 */
public interface L6007ReportService {

    /**
     * Get the form output information to generate a form [Packing Header (FRT)] a.
     * <br/>帳票出力情報を取得し、帳票【 Packing Header(FRT) 】を生成します。
     * <pre>
     ...
     *   Call the VtRpPackHdrForFrtService.searchByCondition (VtRpPackHdrForFrtCriteriaDomain) as an argument criteriaDomain.
     ...
     *       
     * - パッキングヘッダデータの取得
     *       criteriaDomainを引数に{@link VtRpPackHdrForFrtService#searchByCondition(VtRpPackHdrForFrtCriteriaDomain)}を呼び出します。
     *       取得件数が0であれば、エラーとします。
     *       
     * - 帳票の生成
