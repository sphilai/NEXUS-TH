...
/**
 * The implementation class for the Web Call Service (WS7004)
 * <br />Webサービス(WS7004)呼出しサービスの実装クラスです。
 * 
 * @author $Author: 810833613 $
 * @version $Revision: 7053 $
 */
public class Ws7004RestServiceImpl implements Ws7004RestService {
    ...
    /**
     * {@inheritDoc}
     * @see com.globaldenso.eca0027.core.business.ws.Ws7004RestService#deleteShippingAct(java.lang.String, java.lang.String)
     */
    public ResultDomain deleteShippingAct(String compCd, String invoiceNoDelete) throws ApplicationException {

        String webServicePath = "ws7004";
        WebServiceCallerRestDomain wsCallerRestDomain
            = commonService.searchCigmaWebServiceCallerRestDomain(compCd, webServicePath);

        // パラメータを設定する
        wsCallerRestDomain.setParamMap("invoiceNoDelete", invoiceNoDelete);

        try {
            // Webサービスを呼び出す
            ResultDomain resultDomain
                = WebServiceCallerRest.delete(wsCallerRestDomain, new GenericType<ResultDomain>(){});
