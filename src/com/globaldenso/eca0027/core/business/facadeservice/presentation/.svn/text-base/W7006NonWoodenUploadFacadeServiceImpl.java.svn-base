...
/**
 * 
 * A Service for Facade NonWoodenUpload action of Shipping Document Upload screen.
 * <br />船積書類アップロード画面のNonWoodenUploadアクションに対するFacadeサービスです。
 *
 * @author $Author: 810833613 $
 * @version $Revision: 7765 $
 */
public class W7006NonWoodenUploadFacadeServiceImpl extends AbstractCreateFacadeService<W7006UpdateDomain, W7006UpdateDomain> {
     ...
     * デフォルトコンストラクタです。
     */
    public W7006NonWoodenUploadFacadeServiceImpl() {
        super();
    }
    ...
    protected void validateConsistency(W7006UpdateDomain updateDomain)
        ...
        if (!w7001ShippingActService.validateConsistencyFilePathInput(criteriaDomain.getUploadNonWdFileName())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("w7006CriteriaDomain.nonWdFileData", NXS_E7_0011, null)));
        }
        
        if (!w7001ShippingActService.validateConsistencyFileExistence(criteriaDomain.getUploadNonWdFileSize())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("w7006CriteriaDomain.nonWdFileData", GSCM_E0_0036, null)));
        }

        if (!w7001ShippingActService.validateConsistencyFileSize(criteriaDomain.getUploadNonWdFileSize())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("w7006CriteriaDomain.nonWdFileData", NXS_E7_0057, null)));
        }
        
        if (!w7001ShippingActService.validateConsistencyFileName(criteriaDomain.getUploadNonWdFileName())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("w7006CriteriaDomain.nonWdFileData", NXS_E7_0067, null)));
        }
    }
     ...
     *    Call {@link W7001ShippingActService#createOnOtherDocUploadIndividualNonWoodenUpload}
     * </pre>
     * <pre>
     * ‐船積実績サービスの条件ドメイン作成
     ...
     * 
     * ‐ファイルをアップロード、再検索
     *     {@link W7001ShippingActService#createOnOtherDocUploadIndividualNonWoodenUpload}を呼出す
     * </pre>
    ...
    protected W7006UpdateDomain callServices(W7006UpdateDomain updateDomain)
        ...
        W7001ShippingActDomain shippingAct = w7001ShippingActService.createOnOtherDocUploadIndividualNonWoodenUpload(shippingActCriteria);
     ...
     *
     * @param updateDomain Domain conditions of shipment actuality Main screen<br />
     * 船積実績Main画面の条件ドメイン
     ...
     * 画面から渡された検索条件ドメイン
     * @param shippingActDomain Shipment actuality domain<br />
     * 船積実績ドメイン
