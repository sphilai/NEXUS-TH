...
/**
 * This is an implement class of the Facade service to SaveAndMoveToRegister action of a CML PKG Info Detail screen.
 * <br />CML PKG Info Detail画面のSaveAndMoveToRegisterアクションに対するFacadeサービスの実装クラスです。
 * <pre>
 * bean W2013SaveAndMoveToRegisterFacadeService
 * </pre>
 *
 * @author $Author: 400616000304 $
 * @version $Revision: 11271 $
 */
public class W2013SaveAndMoveToRegisterFacadeServiceImpl extends AbstractTransactFacadeService<Void, W2013UpdateDomain> {
     ...
     * constructor.
     */
    public W2013SaveAndMoveToRegisterFacadeServiceImpl() {
    }
    ...
    protected void validateConsistency(W2013UpdateDomain updateDomain)
        ...
        // The check of item number item, and the minimum QUANTITY VALUE is carried out.
        // 品番項目と最少数量値のチェックを実施する。
        if (updateDomain.getListDomainList() == null || updateDomain.getListDomainList().isEmpty()) {
            // When there is no list data
            // 一覧データがない場合
            return;
        ...
        for (W2013ListDomain w2013ListDomain : updateDomain.getListDomainList()) {
            ...
            if (!w2009CmlService.validateConsistencyDetailPackageItemNoQTY(pkgItemNo, qty, mixTagNo)) {
                ...
                if (CheckUtil.isBlankOrNull(pkgItemNo)) {
                    MessageDomain errMsg = new MessageDomain(String.format("listDomainList[%d].pkgItemNo", i), NXS_E7_0068, params);
                    errorList.add(errMsg);
                } else {
                    MessageDomain errMsg = new MessageDomain(String.format("listDomainList[%d].qty", i), NXS_E7_0068, params);
                    errorList.add(errMsg);
                }
            }
            ...
            if (!w2009CmlService.validateConsistencyDetailQTY(qty, mixTagNo)) {
                String[] params  = { pkgItemNo };
                MessageDomain errMsg = new MessageDomain(String.format("listDomainList[%d].qty", i) , NXS_E7_0087, params);
                errorList.add(errMsg);
            }
