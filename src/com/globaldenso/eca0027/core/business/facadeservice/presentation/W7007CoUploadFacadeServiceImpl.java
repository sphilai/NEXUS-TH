...
/**
 * 
 * Facade service for C/O Upload action of B/L upload.
 * <br />B/LアップロードのC/O Uploadアクションに対するFacadeサービスです。
 *
 * @author $Author: 400616000304 $
 * @version $Revision: 11629 $
 */
public class W7007CoUploadFacadeServiceImpl extends AbstractCreateFacadeService<W7007UpdateDomain, W7007UpdateDomain> {
     ...
     * デフォルトコンストラクタです。
     */
    public W7007CoUploadFacadeServiceImpl() {
        super();
    }
    ...
    protected void validateConsistency(W7007UpdateDomain updateDomain)
        ...
        if (!w7001ShippingActService.validateConsistencyFilePathInput(criteriaDomain.getUploadCoFileName())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("coFileData", NXS_E7_0011, null)));
        }
        
        if (!w7001ShippingActService.validateConsistencyFileExistence(criteriaDomain.getUploadCoFileSize())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("coFileData", GSCM_E0_0036, null)));
        }

        if (!w7001ShippingActService.validateConsistencyFileSize(criteriaDomain.getUploadCoFileSize())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("coFileData", NXS_E7_0057, null)));
        }
        
        if (!w7001ShippingActService.validateConsistencyFileName(criteriaDomain.getUploadCoFileName())) {
            throw new ValidationException(Arrays.asList(new MessageDomain("coFileData", NXS_E7_0067, null)));
        }
    }
     ...
     * {@inheritDoc}
     * <pre>
     * - Locking and get number chekck for invoice.
     *  Call  {@link W7001ShippingActService#validateDatabaseTtInvoice(String, String, Date)}.
     * - Shipping documents existence chekck.
     *  Call  {@link W7001ShippingActService#validateDatabaseTtShippingDocumentOnOtherDocUploadSummaryCoUpload}.   
     * <pre>
     * ‐インボイス ロック＆取得件数チェック
     *     {@link W7001ShippingActService#validateDatabaseTtInvoice(String, String, Date)}を呼出す
     * 
     * ‐船積書類 存在チェック
     *     {@link W7001ShippingActService#validateDatabaseTtShippingDocumentOnOtherDocUploadSummaryCoUpload}を呼出す
     * </pre>
    ...
    protected void validateDatabase(W7007UpdateDomain updateDomain)
        ...
        for (int i = 0; i < listDomainList.size(); i++) {
            ...
            // 船積書類 存在チェック
            if (!w7001ShippingActService.validateDatabaseTtShippingDocumentOnOtherDocUploadSummaryCoUpload(shipperCd, invoiceNo, invoiceIssueDt)) {
                Object[] params = {
                    commonService.getResource(criteriaDomain.getLocale(), "label.co")
                };
     ...
     * - File upload and search again.
     * Call {@link W7001ShippingActService#createOnOtherDocUploadSummaryCoUpload}.
     * </pre>
     * <pre>
     * ‐船積実績サービスの条件ドメイン作成
     ...
     * 
     * ‐ファイルをアップロード、再検索
     *     {@link W7001ShippingActService#createOnOtherDocUploadSummaryCoUpload}を呼出す
     * </pre>
    ...
    protected W7007UpdateDomain callServices(W7007UpdateDomain updateDomain) 
        ...
        W7001ShippingActDomain shippingActDomain = w7001ShippingActService.createOnOtherDocUploadSummaryCoUpload(shippingActCriteria);
     ...
     *
     * @param updateDomain Mass update domain for shipping actuality Main screen<br />
     * 船積実績Main画面の一括更新ドメイン
     * @return Conditions domain of Shipping actuality service<br />
     * 船積実績サービスの条件ドメイン
     */
