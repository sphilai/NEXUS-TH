 ...
 * テーブル概要：Export Request Detail Kit/??????KIT?  As line information request KIT export goods , table to hold the information of KIT components unit/????KIT??????????KIT?????????????????
 * 
 ...
 * テーブル概要：Export Request Detail Kit/??????KIT?  As line information request KIT export goods , table to hold the information of KIT components unit/????KIT??????????KIT?????????????????<BR>
 * 
 * <BR>
 ...
 * @version $Revision$
 */
public class W1006ExpRequestDtlKitCriteriaDomain extends AbstractDomain implements Serializable {
     ...
     * デフォルトコンストラクタ。
     */
    public W1006ExpRequestDtlKitCriteriaDomain() {
    }
